...
exports.merge = async function merge(options) {
  ...
  const earliestSuitStart = reports.map(report => new Date(report.stats.end)).sort((a, b) => a - b)[0]
  const latestSuiteEnd = reports.map(report => new Date(report.stats.end)).sort((a, b) => b - a)[0]
  const totalSeconds = Math.round((latestSuiteEnd.getTime() - earliestSuitStart.getTime()) / 1000)
  const totalMinutes = Math.floor(totalSeconds / 60);
  const seconds = totalSeconds % 60;
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;
  const totalTime = hours + 'hrs' + minutes + 'min' + seconds + 'sec';
